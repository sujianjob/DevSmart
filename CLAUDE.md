...
**关键概念**：
- **智能识别**：自动检测代码仓库类型（前端/后端/全栈），用户只需提供路径，无需区分
- **Subagent 并行分析**：使用 Task 工具启动多个 subagent 并发分析代码，提高效率
- **输入依赖**：需要 admin-reverse-docs 输出 + 代码仓库路径（支持多个，自动识别）
- **支持的技术栈**：Vue 2/3、React、Spring Boot、Express、FastAPI
- **5 阶段工作流**：初始化识别 → 模块选择 → 并行分析生成 → 汇总生成 → 审核清单
- **深度分析**：完整阅读代码，追踪调用链，提取业务规则、状态流转、异常处理
- **统一审核**：所有模块生成完成后，输出完整 Checklist 供用户一次性确认
- **输出位置**：在 admin-reverse-docs 目录下新增 `05_产品需求/` 子目录
- **断点续生成**：自动生成 `.prd-state.json` 状态文件（含仓库识别信息）
